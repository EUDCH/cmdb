...
| Secrets | `/opt/cmdb/.env` (deploy user:deploy user, mode 0600 — i.e. `ubuntu:ubuntu` for the canonical deployment; root-owned 0600 would block `docker compose` from reading it, since compose runs as the deploy user). Operator copies from `infra/vm/.env.example` and fills values out-of-band. Never in git, never in repo `secrets.*` except for the SSH deploy key + pinned `known_hosts`. |
