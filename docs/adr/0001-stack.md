...
- Self-hosted on Example PaaS — no vendor / SaaS dependency.
...
| Deployment | Example PaaS host, `systemd` unit running Bun, Caddy or Traefik in front for TLS, Postgres co-located or Example PaaS-managed |
...
- Example PaaS likely already runs PG for other OPERAS services — operationally aligned.
...
- Stack composable with existing OPERAS infrastructure (Example PaaS, OPERAS ID).
...
- The Example PaaS hostname, the systemd unit name, the TLS reverse proxy choice (Caddy vs Traefik), and the Postgres deployment shape (co-located vs Example PaaS-managed) all land in `docs/HANDOVER.md` once decided. None of those affect the stack itself.
